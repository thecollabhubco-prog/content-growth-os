-- ============================================================
-- Content Growth OS — Fixed Database Schema for Supabase
...
CREATE TABLE IF NOT EXISTS humanization_results (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  content_item_id UUID NOT NULL REFERENCES content_items(id) ON DELETE CASCADE,
  workspace_id UUID NOT NULL REFERENCES workspaces(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS trend_signals (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  workspace_id UUID NOT NULL REFERENCES workspaces(id) ON DELETE CASCADE,
  source TEXT CHECK (source IN ('google_trends', 'reddit', 'competitor', 'news', 'social', 'tavily')),
  topic TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_knowledge_items_type ON knowledge_items(workspace_id, type);
...
CREATE INDEX IF NOT EXISTS idx_content_items_type ON content_items(workspace_id, type);
CREATE INDEX IF NOT EXISTS idx_content_items_status ON content_items(workspace_id, status);
CREATE INDEX IF NOT EXISTS idx_content_items_platform ON content_items(workspace_id, platform);
CREATE INDEX IF NOT EXISTS idx_content_items_scheduled ON content_items(scheduled_at) WHERE scheduled_at IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_content_items_created ON content_items(workspace_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_calendar_entries_workspace ON calendar_entries(workspace_id);
CREATE INDEX IF NOT EXISTS idx_calendar_entries_date ON calendar_entries(workspace_id, scheduled_date);
CREATE INDEX IF NOT EXISTS idx_platform_connections_workspace ON platform_connections(workspace_id);
CREATE INDEX IF NOT EXISTS idx_publish_attempts_content ON publish_attempts(content_item_id);
CREATE INDEX IF NOT EXISTS idx_content_performance_workspace ON content_performance(workspace_id);
CREATE INDEX IF NOT EXISTS idx_content_performance_date ON content_performance(workspace_id, metrics_date DESC);
CREATE INDEX IF NOT EXISTS idx_analytics_snapshots_workspace ON analytics_snapshots(workspace_id, platform, snapshot_date DESC);
...
-- ============================================================
-- HELPER FUNCTION (in public schema — no auth schema needed)
-- ============================================================
...
CREATE POLICY "workspace_access" ON workspaces
  FOR ALL USING (id IN (SELECT public.user_workspace_ids()));
...
CREATE POLICY "prompt_library_access" ON prompt_library
  FOR ALL USING (workspace_id IN (SELECT public.user_workspace_ids()));

CREATE POLICY "research_sessions_access" ON research_sessions
  FOR ALL USING (workspace_id IN (SELECT public.user_workspace_ids()));
...
CREATE POLICY "content_versions_access" ON content_versions
  FOR ALL USING (content_item_id IN (SELECT id FROM content_items WHERE workspace_id IN (SELECT public.user_workspace_ids())));

CREATE POLICY "generated_images_access" ON generated_images
  FOR ALL USING (workspace_id IN (SELECT public.user_workspace_ids()));

CREATE POLICY "calendar_entries_access" ON calendar_entries
  FOR ALL USING (workspace_id IN (SELECT public.user_workspace_ids()));

CREATE POLICY "platform_connections_access" ON platform_connections
  FOR ALL USING (workspace_id IN (SELECT public.user_workspace_ids()));

CREATE POLICY "publish_attempts_access" ON publish_attempts
  FOR ALL USING (workspace_id IN (SELECT public.user_workspace_ids()));

CREATE POLICY "humanization_results_access" ON humanization_results
  FOR ALL USING (workspace_id IN (SELECT public.user_workspace_ids()));

CREATE POLICY "content_performance_access" ON content_performance
  FOR ALL USING (workspace_id IN (SELECT public.user_workspace_ids()));

CREATE POLICY "learning_insights_access" ON learning_insights
  FOR ALL USING (workspace_id IN (SELECT public.user_workspace_ids()));

CREATE POLICY "trend_signals_access" ON trend_signals
  FOR ALL USING (workspace_id IN (SELECT public.user_workspace_ids()));

CREATE POLICY "analytics_snapshots_access" ON analytics_snapshots
  FOR ALL USING (workspace_id IN (SELECT public.user_workspace_ids()));
...
CREATE POLICY "email_drafts_access" ON email_drafts
  FOR ALL USING (workspace_id IN (SELECT public.user_workspace_ids()));

CREATE POLICY "audit_logs_access" ON audit_logs
  FOR ALL USING (workspace_id IN (SELECT public.user_workspace_ids()));

CREATE POLICY "automation_runs_access" ON automation_runs
  FOR ALL USING (workspace_id IN (SELECT public.user_workspace_ids()));
...
CREATE OR REPLACE TRIGGER update_workspaces_updated_at BEFORE UPDATE ON workspaces FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
CREATE OR REPLACE TRIGGER update_content_items_updated_at BEFORE UPDATE ON content_items FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
CREATE OR REPLACE TRIGGER update_platform_connections_updated_at BEFORE UPDATE ON platform_connections FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
CREATE OR REPLACE TRIGGER update_calendar_entries_updated_at BEFORE UPDATE ON calendar_entries FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
CREATE OR REPLACE TRIGGER update_knowledge_items_updated_at BEFORE UPDATE ON knowledge_items FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
CREATE OR REPLACE TRIGGER update_user_profiles_updated_at BEFORE UPDATE ON user_profiles FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();

-- ============================================================
-- AUTO-CREATE USER PROFILE ON SIGNUP
-- (Uses Supabase's allowed trigger on auth.users)
-- ============================================================
